...
export async function getDesignScore(productDescription: string, iconAnalysis: string): Promise<{ designScore: number; scoreExplanation: string }> {
  try {
    const response = await fetch('http://localhost:3001/api/analyze-score', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Failed to parse error response' }));
      throw new Error(errorData.error || `Failed to get design score: ${response.status}`);
    }
